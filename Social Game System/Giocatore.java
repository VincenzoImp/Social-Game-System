...
public class Giocatore extends Center {
	...
	public boolean AggiornaGiocatore() throws InterruptedException {
		if (umore>=100) {
			...
			if (io==this && figli == LimiteFigli) {
				System.out.println("Sei molto felice, ma l'eta' avanza e non puoi più avere figli!");
				umore -= 50;
	...
	private void Morte() throws InterruptedException {
		if (this.carattere.carattere.equals("player")) {
			if (!MortiViventi.contains(this)) {
				if (umore<=0) System.out.println("Mi dispiace " + this.nome + " " + this.cognome + ", sei morto prematuramente di tristezza! Il gioco termina qui");
				else System.out.println("Mi dispiace " + this.nome + " " + this.cognome + ", la tua ora e' giunta, sei morto! Il gioco termina qui");
			}
			isalive=false;
		}
	...
	private void Nascita() throws InterruptedException {
		...
		if (cimitero.isEmpty()) {
			...
			if (carattere.carattere.equals("player")) Creafiglio(figlio);
			else {
	...
	private void Creafiglio(Giocatore figlio) {
	...
	private void Assimila_messaggio(int messaggio, Giocatore mittente) throws InterruptedException {
		...
		if (this==io) aggiornagraficoumore();
	}
	...
	public void Crea_Amicizia(Giocatore giocatore) throws InterruptedException {
		...
		if (a >= 0 && b >= 0) {
			if (interazioni.containsKey(giocatore) == false) {
				if (giocatore.carattere.carattere.equals("player")) System.out.println("Hai fatto amicizia con " + this.nome + " " + this.cognome + ", e' un tipo " + this.carattere.carattere);
				if (this.carattere.carattere.equals("player")) System.out.println("Hai fatto amicizia con " + giocatore.nome + " " + giocatore.cognome + ", e' un tipo " + giocatore.carattere.carattere);
