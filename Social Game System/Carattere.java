...
public class Carattere {
	...
	private int Player_input(int messaggio, Giocatore mittente, Giocatore self) {
		...
		if (self.interazioni.get(mittente).storico.size()==0) {
			System.out.println("Prima di oggi non vi siete scambiati messaggi");
		}
		else if (self.interazioni.get(mittente).storico.size()<=10) {
			System.out.println("Questi sono gli ultimi messaggi che ti ha inviato --> " + self.interazioni.get(mittente).storico);
		}
		else {
			for (int i = 0; i < 10; i++) {
				storicolimitato.add(self.interazioni.get(mittente).storico.get(i));
			}
			System.out.println("Questi sono gli ultimi messaggi che ti ha inviato --> " + storicolimitato);
		}

		continua=true;
		do {
			try {
				System.out.println("Come lo vuoi interpretare? (-7<=interpretazione<=7)");
	...
	private int Player_output(Giocatore destinatario, Giocatore self) {
		...
		int significato=0;	    
		LinkedList<Integer> storicolimitato = new LinkedList<Integer>();
		System.out.println("Invia un messaggio a " + destinatario.nome + " " + destinatario.cognome + ", per te questa persona ha importanza: " + self.interazioni.get(destinatario).importanza);
		if (self.interazioni.get(destinatario).storico.size()==0) {
			System.out.println("Prima di oggi non vi siete scambiati messaggi");
		}
		else if (self.interazioni.get(destinatario).storico.size()<=10) {
			System.out.println("Questi sono gli ultimi messaggi che ti ha inviato --> " + self.interazioni.get(destinatario).storico);
		}
		else {
			for (int i = 0; i < 10; i++) {
		...
		do {
			try {
				System.out.println("Che messaggio vuoi mandare? (-5<=messaggio<=5)");
				significato = inputScanner.nextInt();
	...
	private void Player_AggiornaImportanza(Giocatore conoscente, Giocatore self) {
		LinkedList<Integer> storicolimitato = new LinkedList<Integer>();
		System.out.println("Questi sono stati gli ultimi messaggi che ti ha mandato " + conoscente.nome + " " + conoscente.cognome);
		if (self.interazioni.get(conoscente).storico.size()<=10) {
			System.out.println(" --> " + self.interazioni.get(conoscente).storico);
		}
		else {
			for (int i = 0; i < 10; i++) {
				storicolimitato.add(self.interazioni.get(conoscente).storico.get(i));
			}
			System.out.println(" --> " + storicolimitato);
		}
		...
		do {
			try {
				System.out.println("incrementa-decrementa importanza (-0,5<valore<0,5)");
