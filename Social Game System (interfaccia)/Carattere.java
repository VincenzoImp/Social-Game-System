...
public class Carattere {
	...
	private int Player_input(int messaggio, Giocatore mittente, Giocatore self) {
		...
		Center.frame.textArea.append(mittente.nome + " " + mittente.cognome + " ti ha mandato questo messaggio --> " + messaggio + " (Tradotto = " + significato + ")\n");
		if (self.interazioni.get(mittente).storico.size()==0) {
			Center.frame.textArea.append("Prima di oggi non vi siete scambiati messaggi\n");
		}
		else if (self.interazioni.get(mittente).storico.size()<=10) {
			Center.frame.textArea.append("Questi sono gli ultimi messaggi che ti ha inviato --> " + self.interazioni.get(mittente).storico + "\n");
		}
		else {
			for (int i = 0; i < 10; i++) {
				storicolimitato.add(self.interazioni.get(mittente).storico.get(i));
			}
			Center.frame.textArea.append("Questi sono gli ultimi messaggi che ti ha inviato --> " + storicolimitato + "\n");
		}

		continua=true;
		do {
			try {
				Center.frame.InputDati.setText("Come lo vuoi interpretare? (-7<=interpretazione<=7)");
	...
	private int Player_output(Giocatore destinatario, Giocatore self) {
		...
		int significato=0;	    
		LinkedList<Integer> storicolimitato = new LinkedList<Integer>();
		Center.frame.textArea.append("Invia un messaggio a " + destinatario.nome + " " + destinatario.cognome + ", per te questa persona ha importanza: " + self.interazioni.get(destinatario).importanza + "\n");
		if (self.interazioni.get(destinatario).storico.size()==0) {
			Center.frame.textArea.append("Prima di oggi non vi siete scambiati messaggi\n");
		}
		else if (self.interazioni.get(destinatario).storico.size()<=10) {
			Center.frame.textArea.append("Questi sono gli ultimi messaggi che ti ha inviato --> " + self.interazioni.get(destinatario).storico + "\n");
		}
		else {
			for (int i = 0; i < 10; i++) {
		...
		do {
			try {
				Center.frame.InputDati.setText("Che messaggio vuoi mandare? (-5<=messaggio<=5)");
				significato = inputScanner.nextInt();
	...
	private void Player_AggiornaImportanza(Giocatore conoscente, Giocatore self) {
		LinkedList<Integer> storicolimitato = new LinkedList<Integer>();
		Center.frame.textArea.append("Questi sono stati gli ultimi messaggi che ti ha mandato " + conoscente.nome + " " + conoscente.cognome);
		if (self.interazioni.get(conoscente).storico.size()<=10) {
			Center.frame.textArea.append(" --> " + self.interazioni.get(conoscente).storico + "\n");
		}
		else {
			for (int i = 0; i < 10; i++) {
				storicolimitato.add(self.interazioni.get(conoscente).storico.get(i));
			}
			Center.frame.textArea.append(" --> " + storicolimitato + "\n");
		}
		...
		do {
			try {
				Center.frame.InputDati.setText("incrementa-decrementa importanza (-0,5<valore<0,5)" );
